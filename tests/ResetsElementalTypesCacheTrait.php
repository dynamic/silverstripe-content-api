 ...
 *
 * `getElementalTypes()` caches its result in a static on branch `2`'s
 * elemental (6.2+ only) — a leftover cache entry there can leak between
 * tests unless cleared via `::reset()`. That static cache doesn't exist at
 * all pre-6.2, including every 5.x release this branch depends on, so
 * `::reset()` itself is undefined there — the `method_exists()` guard makes
 * this call branch-neutral: a real reset on branch `2`, a correct no-op
 * here (nothing was ever cached to leak).
 */
