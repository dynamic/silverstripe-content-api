...
| `UNPUBLISH_STRANDS_DESCENDANTS` | 409 | `unpublish` (stage action or batch `delete` with `mode: "unpublish"`) would remove still-live `Hierarchy` descendants whose draft parent has already moved elsewhere — pass `force: true` to bypass and accept the loss (#71) |
